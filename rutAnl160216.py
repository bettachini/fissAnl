...
# Funciones auxiliares - basadas en las de anl150807
...
def dataPathFile(tipo, mes, dia, acq):
    '''
    (str, int, int, int) -> string
    tipo:  alterativamente 'acq' (neutrones) o 'pul' (pulsos generador)
    mes: int
    ...
    '''
    path= './data/'
    fNS= tipo+ '15'+ '{:02n}'.format(mes)+ '{:02n}'.format(dia)+ 'z'
    fNE= '.npy.npz'
    return path+ fNS+ '{:02n}'.format(acq)+fNE
...
def tauCeti3eAreaMax(tipo, T, Meas2, messungenZahl, prop, conv1, rechazo=0.1):
    '''
...
def tauCeti4(tipo, T, Meas2, messungenZahl, prop, conv1, rechazo=0.1):
    '''
    Calcula índices risetime como tiempo entre 10% y 90% de excursión entre nivel de base y máximo.
    Retorna area por sobre nivel de base de AMC entre 10% a izquierda y derecha.
    ...
    if ((bMeas2[3])>np.abs(bMeas2[1]* rechazo)):
        ...
        if (tipo=='acq'):
            ...
            # fwhm
            mitadFWHM= 0.5* (maxCh1+ bMeas2[0])
            izqFWHM= ixTau[2]
            derFWHM= ixTau[0]
            while (Meas2[0][izqFWHM]> mitadFWHM): # para ancho en Pulser
                izqFWHM-=1
            while (Meas2[0][derFWHM]> mitadFWHM): # para ancho en Pulser
                derFWHM+=1
            anchoPulser= T[derFWHM ]- T[izqFWHM ]
            # ch1>10% sobre línea de base [ancho Pulser]
            while (Meas2[0][ixTau[2]]> v10[0]): # para ancho en Pulser
                ixTau[2]+= 1        
            #while (Meas2[0,ixTau[2]]> v90[0]): # para tau en FLUC
...
class conv(object):
    __slots__ = ['ch1Cero', 'ch1Paso', 'ch2Cero', 'ch2Paso', 'escalaCh1VoltsDiv', 'escalaCh2VoltsDiv', 'ceroCh1Div', 'ceroCh2Div']
...
def parametrosConversion(npzData):
    ...
    divVert= 4 # numero de divisiones verticales (positivas)
    divHoriz= 10
    ptosVert= 100 # 8 bits definición vertical , pero la división +4 = pto 100 (-4 = -100), los límites son +127,-127 y con el cero son 2**8-ptos
    ptosHoriz= 2500
    ...
    # Vertical (potencial)
    conv1= conv()
    conv1.escalaCh1VoltsDiv= settingsList['SCALE1'] # [V/div]
    conv1.ch1Paso= conv1.escalaCh1VoltsDiv* divVert/ ptosVert # [V/div]
    conv1.escalaCh2VoltsDiv= settingsList['SCALE2'] # [V/div]
    conv1.ch2Paso= conv1.escalaCh2VoltsDiv* divVert/ ptosVert # [V/div]
    conv1.ceroCh1Div= settingsList['POSITION1'] # [div]
    conv1.ch1Cero= conv1.ceroCh1Div* divVert/ ptosVert # [ptos]
    conv1.ceroCh2Div= settingsList['POSITION2'] # [div]
    conv1.ch2Cero= conv1.ceroCh2Div* ptosVert/ divVert # [ptos]
    # print(escalaCh1VoltsDiv, escalaCh2VoltsDiv, ceroCh1Div, ceroCh2Div)
    # print(conv1.ch1Paso, conv1.ch2Paso, conv1.ch1Cero, conv1.ch2Cero)    
    return conv1
...
def conversionV2(conv1, T, areaPulser, maxPulser, areaSubidaAMC, areaTotalAMC, maxAMC):
    ...
    areaPulser*= pasoT* 4E-2* conv1.escalaCh1VoltsDiv 
    # areaPulser*= pasoT* conv1.ch1Paso
    maxPulser= (4E-2* maxPulser+ conv1.ceroCh1Div)* (-1)* conv1.escalaCh1VoltsDiv
    # maxPulser= (maxPulser- conv1.ch1Cero)* conv1.ch1Paso
    areaSubidaAMC*= pasoT* 4E-2* conv1.escalaCh2VoltsDiv
    # areaSubidaAMC*= pasoT* conv1.ch2Paso
    areaTotalAMC*= pasoT* 4E-2* conv1.escalaCh2VoltsDiv
    # areaTotalAMC*= pasoT* conv1.ch2Paso
    maxAMC= (4E-2* maxAMC- conv1.ceroCh2Div)* conv1.escalaCh2VoltsDiv
...
def plota3(conv1, T, Meas2, messungenZahl, prop, bMeas2, maxIx, ixTau, izqFWHM, derFWHM):
    '''
    ...
    # print('¿Entonces?') 
    # print(str(conv1.ch1Cero)) 
    # print(str(conv1.ch1Paso)) 
    ...
    ## marks maxima
    ax0.scatter(T[maxIx[0]], (4E-2* Meas2[0,:][maxIx[0]]+ conv1.ceroCh1Div)* ((-1)* conv1.escalaCh1VoltsDiv), marker='o')
    ax1.scatter(T[maxIx[1]], (4E-2* Meas2[1,:][maxIx[1]]- conv1.ceroCh2Div)* conv1.escalaCh2VoltsDiv, marker='o')
    ax1.scatter(T[maxIx[1]], (4E-2* Meas2[1,:][maxIx[1]]- conv1.ceroCh2Div)* conv1.escalaCh2VoltsDiv, marker='o')
    ## ch1 fwhm
    ax0.scatter(T[izqFWHM], (4E-2* Meas2[0][izqFWHM]+ conv1.ceroCh1Div)* ((-1)* conv1.escalaCh1VoltsDiv), marker='o')
    ax0.scatter(T[derFWHM], (4E-2* Meas2[0][derFWHM]+ conv1.ceroCh1Div)* ((-1)* conv1.escalaCh1VoltsDiv), marker='o')
    ## risetime ch1
    ch1Erste= (4E-2* Meas2[0,:][ixTau[0]]+ conv1.ceroCh1Div)* ((-1)* conv1.escalaCh1VoltsDiv)
    ch1Letzte= (4E-2* Meas2[0,:][ixTau[2]]+ conv1.ceroCh1Div)* ((-1)* conv1.escalaCh1VoltsDiv)
    ax0.scatter(T[ixTau[0]], ch1Erste, marker='o')
    ax0.scatter(T[ixTau[2]], ch1Letzte, marker='o')
    ## risetime ch2
    ch2percent10= (4E-2* Meas2[1,:][ixTau[1]]- conv1.ceroCh2Div)* conv1.escalaCh2VoltsDiv
    ch2percent90= (4E-2* Meas2[1,:][ixTau[3]]- conv1.ceroCh2Div)* conv1.escalaCh2VoltsDiv
...
# def plota2(ceroCh2Pto, pasoVerticalCh2VoltPto, ceroCh1Pto, pasoVerticalCh1VoltPto, T, Meas2, messungenZahl, prop):
    ...
    fig.subplots_adjust(hspace=0.000)
    # Axis 0
    ax0.plot(T, (Meas2[0,:]+ conv1.ch1Cero)* conv1.ch1Paso, 'y')  # puesto que ch1= ch1* ()
...
# agregador area pulsos del generador
def agregador2(tipo, anne, mes, dia, rango, prop= 0.25, rechazo= 0.09):
    if (tipo=='pul'):
    ...
    for i in rango:
        ...
        if (tipo=='pul'):
            # print('pasa por agregador2: for : pul')
            ...
            tauAMC, areaSubidaAMC, areaTotalAMC, maxAMC= tausAreasMaxSerieAdq(tipo, acq, prop, rechazo)
    ...
    if (tipo=='pul'):
        ...
        return tausAMC, areasTotalAMC, areasSubidaAMC, maximosAMC
...
def todoEnmascarado3(tipo, mascara, anchosPulser, areasPulser, maximosPulser, tausAMCComprimidos, areasTotalAMC, areasSubidaAMC, maximosAMC):
    resultados=resultadosComprimido()
